...
static void *coalesce(struct myheap *h, void *first_block_start) {
  // STUDENT DONE
  long* next_block = get_next_block(first_block_start);

  // either blocks are in use or given block is the last block
  if (block_is_in_use(first_block_start) || block_is_in_use(next_block) || (is_last_block(h, first_block_start))) {
...
static int get_size_to_allocate(int user_size) {
  // STUDENT DONE
  // Padding is difference needed to make user_size a multiple of header_size
  int padding = (HEADER_SIZE - (user_size % HEADER_SIZE));
  // need to add padding, requested payload size, and 2 * header size for header and footer
  int new_size = padding + user_size + 2 * HEADER_SIZE;
  return new_size;
}
...
static void *split_and_mark_used(struct myheap *h, void *block_start, int needed_size) {
  // STUDENT DONE
  int needed_block_size = get_size_to_allocate(needed_size); // need to pad the size for alignment
  int curr_block_size = get_block_size(block_start);
  int excess_blocks = curr_block_size - needed_block_size; // used to determine if we have enough excess to create a second block
  int split_size = 3 * HEADER_SIZE; // min payload = 4 but must be alligned to 8 bytes

  if (excess_blocks >= split_size) { // if (current block size - needed block size >= 
                                     // minimum block size (despite payload being equal to 4 bytes))
    
    set_block_header(block_start, needed_block_size, 1); // resizes first split and sets it to in use
    long *split_block = get_next_block(block_start); // pointer to second half of the split block
    set_block_header(split_block, excess_blocks, 0); // Sets up second half of split black (not in use)
    return get_payload(block_start); // returns pointer to payload of first half of split

  } else {
    set_block_header(block_start, curr_block_size, 1); // block is marked as in use (if it wasn't already)
    return get_payload(block_start); // returns pointer to payload of this block
  }
}
...
void myheap_free(struct myheap *h, void *payload) {
  ...
  long* start_block = get_block_start(payload); // find block start
  int start_size = get_block_size(start_block); // get total block size

  long* prev_block = get_previous_block(start_block); // prev block
  long* next_block = get_next_block(start_block);// next block

  if (is_first_block(h, start_block)) { /* SPECIAL CASE: first block has no prev */

    if (!(block_is_in_use(next_block))) {
      set_block_header(start_block, start_size, 0); // free itself
      coalesce(h, start_block); // connect start block with next block
    } else {
      set_block_header(start_block, start_size, 0); // free itself
    }
    
  } else if (is_last_block(h, start_block)) { /* SPECIAL CASE: last block has no next */

    if (!(block_is_in_use(prev_block))) { // if prev_block is free
      set_block_header(start_block, start_size , 0); // free itself 
      coalesce(h, prev_block); // connect previous block with curr block
    } else {
      set_block_header(start_block, start_size, 0); // free itself
    }

  } else if (!(block_is_in_use(prev_block)) && !(block_is_in_use(next_block))) {  // both neighbors are free

    set_block_header(start_block, start_size, 0); // free itself
    coalesce(h, start_block); // connect curr with next
    coalesce(h, prev_block); // connect prev with curr (which is bigger now)

  } else if (!(block_is_in_use(prev_block))) { // only prev block is free

    set_block_header(start_block, start_size, 0);
    coalesce(h, prev_block);

  } else if (!(block_is_in_use(next_block))) { // only next block is free

    set_block_header(start_block, start_size, 0);
    coalesce(h, start_block);

  } else { // neither neighbors are free
    set_block_header(start_block, start_size, 0);
  }
}
...
void *myheap_malloc(struct myheap *h, unsigned int user_size) {
  ...
  /*
  Method:
  iterate block by block within the heap range until you find a place to allocate
  split off the amount you need to use and mark as used.
  return that
  */

  for (long* start_block = h->start; is_within_heap_range(h, start_block); start_block = get_next_block(start_block)) {
    if (get_payload_size(start_block) >= user_size && !(block_is_in_use(start_block))) {
      //was initially get block size, but changed to payloadsize as we want to fit user size in payload, 
      //also add the block in use clause to make sure block is free
      
      return split_and_mark_used(h, start_block, user_size);
    }
  }
  return NULL;
}
