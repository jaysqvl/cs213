...
int exec() {
  ...
  void *jt[16] = {&&case0x0, &&case0x1, &&case0x2, &&case0x3, &&case0x4 , &&case0x5, &&case0x6, &&case0x7, &&case0x8, &&case0x9, &&case0xa, &&case0xb, &&case0xc, &&case0xd, &&case0xe, &&case0xf};
  void *jt6[16] = {&&case0x60, &&case0x61, &&case0x62, &&case0x63, &&case0x64, &&case0x65, &&case0x66, &&case0x67, &&caseError6, &&caseError6, &&caseError6, &&caseError6, &&caseError6, &&caseError6, &&caseError6, &&case0x6f};

  // if proper instruction
  // Jump table handles 0 - f as valid inputs, if condition handles all invalid conditions (0 < insOpCode > 15)
  if (insOpCode < 0 || insOpCode > 15) goto caseError;
    else goto *jt[insOpCode];
    case0x0: // ld $i, d .............. 0d-- iiii iiii
      reg [insOp0] = insOpExt;
      goto BREAK;
    case0x1: // ld o(rs), rd .......... 1osd
      addr = (insOp0 << 2) + reg [insOp1];
      reg [insOp2] = mem [addr] << 24 | mem [addr+1] << 16 | mem [addr+2] << 8 | mem [addr+3];
      goto BREAK;
    case0x2: // ld (rs, ri, 2), rd .... 2sid
      addr = reg [insOp0] + (reg [insOp1] << 2);
      reg [insOp2] = mem [addr] << 24 | mem [addr+1] << 16 | mem [addr+2] << 8 | mem [addr+3];
      goto BREAK;
    case0x3: // st rs, o(rd) .......... 3sod
    ...
    case0x6: // ALU ................... 6-sd
      ...
      // Jump table handles 7 - f (f is 15) as invalid inputs alongside if condition handling invalid hex characters
      if (insOp0 < 0 || insOp0 > 15) goto caseError6;
      else goto *jt6[insOp0];
        case0x60: // mov rs, rd ........ 60sd
          reg [insOp2] = reg [insOp1];
          goto BREAK6;
        case0x61: // add rs, rd ........ 61sd
          reg [insOp2] = reg [insOp1] + reg [insOp2];
          goto BREAK6;
        case0x62: // and rs, rd ........ 62sd
          reg [insOp2] = reg [insOp1] & reg [insOp2];
          goto BREAK6;
        case0x63: // inc rr ............ 63-r
          reg [insOp2] = reg [insOp2] + 1;
          goto BREAK6;
        case0x64: // inca rr ........... 64-r
          reg [insOp2] = reg [insOp2] + 4;
          goto BREAK6;
        case0x65: // dec rr ............ 65-r
          reg [insOp2] = reg [insOp2] - 1;
          goto BREAK6;
        case0x66: // deca rr ........... 66-r
          reg [insOp2] = reg [insOp2] -4;
          goto BREAK6;
        case0x67: // not ............... 67-r
    ...
    case0x8: // br o .................. 8-oo
      pc += insOpImm << 1;
      goto BREAK;
    case0x9: // beq rs, o ............. 9roo
      if (reg [insOp0] == 0)
        pc += insOpImm << 1;
      goto BREAK;
    case0xa: // bgt rs, o .............. aroo
      if (reg [insOp0] > 0)
        pc += insOpImm << 1;
      goto BREAK;
    case0xb: // j i ................... b--- iiii iiii
      pc = insOpExt;
      goto BREAK;
    case0xc: // j o(rr) ............... croo
